...
private:
    TestResult testVectorOperations() {
        ...
        // Test length
        float len = a.length();
        TEST_ASSERT(std::abs(len - std::sqrt(14.0f)) < 0.0001f, "Vector length incorrect");
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            if (std::abs(result[i] - expected) > 0.0001f) {
                return {__func__, false, 
                        "SIMD multiplication failed at index " + std::to_string(i), 0.0};
            }
        }
    ...
    TestResult testColorConversion() {
        ...
        TEST_ASSERT(std::abs(rgb[0] - 1.0f) < 0.01f, "HSV->RGB red channel incorrect");
        TEST_ASSERT(std::abs(rgb[1] - 0.0f) < 0.01f, "HSV->RGB green channel incorrect");
        TEST_ASSERT(std::abs(rgb[2] - 0.0f) < 0.01f, "HSV->RGB blue channel incorrect");
        ...
        TEST_ASSERT(std::abs(hsv2[1] - 1.0f) < 0.01f, "RGB->HSV saturation incorrect");
...
public:
    std::string getName() const override {
        return "Core Functionality Tests";
    }
