...
public:
  std::string getName() const override { return "Shader Wrapper Tests"; }
...
private:
  TestResult testNoDuplicateUniformBlocks() {
    ...
    if (wrappedFragStart != std::string::npos &&
        fallbackStart != std::string::npos) {
      wrappedSection = wrapperContent.substr(wrappedFragStart,
                                             fallbackStart - wrappedFragStart);
    }

    // Count occurrences of "uniform Uniforms" in the wrappedFrag section only
    std::regex uniformBlockRegex(
        R"(layout\s*\(\s*std140\s*\)\s*uniform\s+Uniforms\s*\{)");
    auto wrapperBegin = std::sregex_iterator(
        wrappedSection.begin(), wrappedSection.end(), uniformBlockRegex);
    ...
    TEST_ASSERT(
        wrapperUniformBlocks == 0,
        "wrappedFrag defines its own Uniforms block - this will cause "
        "duplicate declaration errors when shaders include common.glsl. "
        "Remove the Uniforms block from wrappedFrag and let common.glsl "
        "provide it.");

    return {__func__, true,
            "wrappedFrag correctly does not define Uniforms block - will be "
            "provided by common.glsl",
            0.0};
  }
  ...
  TestResult testCommonGLSLHasUniformBlock() {
    ...
    TEST_ASSERT(std::regex_search(commonContent, uniformBlockRegex),
                "common.glsl must define the Uniforms block since the wrapper "
                "no longer does");

    // Check for all required uniforms
    std::vector<std::string> requiredUniforms = {
        "float time",  "float speed",        "vec2 resolution",
        "vec2 mouse",  "float mouseButtons", "float intensity",
        "vec4 date",   "int frame",          "float deltaTime",
  ...
  TestResult testIncludeSystemNotDuplicatingUniforms() {
    ...
    std::regex uniformVarRegex(
        R"(\b(float|vec2|vec4|int)\s+(time|speed|resolution|mouse|mouseButtons|intensity|date|frame|deltaTime|alpha|gravity)\b)");

    std::set<std::string> foundUniforms;
    auto begin = std::sregex_iterator(commonContent.begin(),
                                      commonContent.end(), uniformVarRegex);
    ...
    for (auto it = begin; it != end; ++it) {
      ...
      TEST_ASSERT(foundUniforms.find(uniformName) == foundUniforms.end(),
                  "Duplicate uniform declaration found in common.glsl: " +
                      uniformName);
    ...
    std::vector<std::string> expectedUniforms = {
        ...
        "date", "frame", "deltaTime",  "alpha", "gravity"};
    ...
    return {__func__, true,
            "No duplicate uniform declarations found, all expected uniforms "
            "present",
            0.0};
  }
  ...
  TestResult testShaderIncludeDepth() {
    ...
    TEST_ASSERT(shaderContent.find("#include \"base/common.glsl\"") !=
                    std::string::npos,
    ...
    TEST_ASSERT(commonContent.find("#include") == std::string::npos,
                "common.glsl should not have #include directives to avoid "
...
#define TWO_PI 6.28318530718
    ...
    std::regex uniformBlockRegex(
        R"(layout\s*\(\s*std140\s*\)\s*uniform\s+Uniforms\s*\{)");
    auto begin = std::sregex_iterator(assembledShader.begin(),
                                      assembledShader.end(), uniformBlockRegex);
    auto end = std::sregex_iterator();
    ...
    TEST_ASSERT(uniformBlockCount == 1,
                "Assembled shader has " + std::to_string(uniformBlockCount) +
                    ...
                    "This causes: error: interface block `Uniforms' with type "
                    "`uniform' already taken");
    ...
    for (const auto &uniform : uniformsToCheck) {
      ...
      TEST_ASSERT(count <= 1, "Duplicate declaration of '" + uniform +
                                  "' found (" + std::to_string(count) +
                                  " occurrences). "
                                  "This causes: error: `'" +
                                  uniform.substr(uniform.find(' ') + 1) +
                                  "' redeclared");
    }

    return {__func__, true,
            "Assembled shader has exactly 1 Uniforms block and no duplicate "
