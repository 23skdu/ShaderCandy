...
fragment float4 fragment_main(VertexOut in [[stage_in]],
    ...
    // Static/custom_noise texture
    float staticNoise = custom_random(uv * 500.0 + t * 100.0);
    ...
    for (float i = 0.0; i < numLights; i++) {
        ...
        float flicker = custom_random(float2(t + i, i)) * 0.5 + 0.5;
        float light = smoothstep(0.3 * flicker, 0.0, dist);
        
        float3 lightCol = float3(1.0, 0.3, 0.1);  // Orange/red
        lightCol = mix(lightCol, float3(1.0, 0.8, 0.0), custom_random(float2(i, t)));  // Yellow
    ...
    float glitch = step(0.95, custom_random(floor(uv * 20.0) + floor(t * 10.0)));
    glitch *= u.treble;
    col = mix(col, float3(1.0), glitch);
    
    // Random scratchy lines
    float2 scratchP = p * 10.0;
    scratchP.x += custom_random(floor(scratchP.y)) * 10.0;
    float scratch = step(0.98, custom_random(floor(scratchP)));
    ...
    // Distressed overlay
    float distress = custom_noise(p * 30.0 + t);
    col *= 0.8 + distress * 0.4;
    
    // Color splatters
    float2 splatP = p * 3.0;
    float splat = custom_noise(splatP + t * 0.5);
    splat = step(0.7, splat);
    float3 splatCol = custom_random(float2(floor(splatP.x), floor(splatP.y)));
