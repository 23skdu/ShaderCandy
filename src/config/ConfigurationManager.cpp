...
std::vector<ShaderConfig> ConfigurationManager::getShadersByCategory(const std::string& category) const {
    std::vector<ShaderConfig> result;
    for (const auto& [name, config] : shaderConfigs_) {
...
ShaderConfig* ConfigurationManager::getShaderConfig(const std::string& shaderName) {
    auto it = shaderConfigs_.find(shaderName);
    return (it != shaderConfigs_.end()) ? &it->second : nullptr;
}

void ConfigurationManager::setParameter(const std::string& shader, const std::string& param, ConfigValue value) {
    parameterValues_[shader][param] = value;
}

ConfigValue ConfigurationManager::getParameter(const std::string& shader, const std::string& param) const {
...
ConfigValue ConfigurationManager::getParameterDefault(const std::string& shader, const std::string& param) const {
...
void ConfigurationManager::resetParameter(const std::string& shader, const std::string& param) {
    auto shaderIt = parameterValues_.find(shader);
    if (shaderIt != parameterValues_.end()) {
...
bool ConfigurationManager::saveToFile(const std::string& path) const {
    try {
        ...
        file << "  \"adaptiveQuality\": " << (settings_.adaptiveQuality ? "true" : "false") << ",\n";
...
void ConfigurationManager::parseShaderMetadata(const std::string& shaderPath) {
    ...
    // In a full implementation, this would parse comments/metadata from shader files
    
    std::string shaderName = std::filesystem::path(shaderPath).stem().string();
    
    // Register a basic config
    ShaderConfig config;
...
std::string ConfigurationManager::getConfigDirectory() {
    ...
    if (home) {
        std::string path = std::string(home) + "/Library/Application Support/ShaderCandy";
        return path;
    }
