...
Preset::Preset(const std::string& shader)
    : Preset() {
    shaderName = shader;
    name = "Custom " + shader;
}

std::map<std::string, std::variant<float, int, bool, std::string>> Preset::toDictionary() const {
    ...
    if (!author.empty()) dict["author"] = author;
    if (!description.empty()) dict["description"] = description;
    if (!createdDate.empty()) dict["created"] = createdDate;
    if (!modifiedDate.empty()) dict["modified"] = modifiedDate;
    if (!category.empty()) dict["category"] = category;
...
Preset Preset::fromDictionary(const std::map<std::string, std::variant<float, int, bool, std::string>>& dict) {
    Preset preset;
...
std::vector<Preset> PresetManager::discoverPresets(const std::string& directory) {
...
std::vector<Preset> PresetManager::allBuiltInPresets() {
    std::string builtInPath = ConfigurationManager::getPresetDirectory() + "/BuiltIn";
...
bool PresetManager::savePreset(const Preset& preset, const std::string& path, std::string& error) {
    // Validate preset
...
std::optional<Preset> PresetManager::loadPreset(const std::string& path, std::string& error) {
    try {
...
std::vector<Preset> PresetManager::importPresets(const std::string& directory, std::string& error) {
    return discoverPresets(directory);
}

bool PresetManager::exportPreset(const Preset& preset, const std::string& outputPath, std::string& error) {
