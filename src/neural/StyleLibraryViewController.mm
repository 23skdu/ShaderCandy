...
@interface StyleLibraryViewController () <NSCollectionViewDataSource, NSCollectionViewDelegate>
...
- (void)setupUI {
    ...
    // Category filter
    _categoryPopup = [[NSPopUpButton alloc] initWithFrame:NSMakeRect(20, self.view.bounds.size.height - 40, 150, 28)];
    ...
    // Favorites button
    _favoritesButton = [[NSButton alloc] initWithFrame:NSMakeRect(180, self.view.bounds.size.height - 40, 100, 28)];
    ...
    // Strength slider
    _strengthSlider = [[NSSlider alloc] initWithFrame:NSMakeRect(300, self.view.bounds.size.height - 40, 200, 28)];
    ...
    _strengthLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(510, self.view.bounds.size.height - 40, 60, 22)];
    ...
    _scrollView = [[NSScrollView alloc] initWithFrame:NSMakeRect(0, 60, self.view.bounds.size.width, self.view.bounds.size.height - 100)];
    _scrollView.autoresizingMask = NSViewWidthSizable | NSViewHeightSizable;
    _scrollView.documentView = _collectionView;
    [self.view addSubview:_scrollView];

    // Apply button
    NSButton *applyButton = [[NSButton alloc] initWithFrame:NSMakeRect(self.view.bounds.size.width - 120, 10, 100, 32)];
    ...
    // Close button
    NSButton *closeButton = [[NSButton alloc] initWithFrame:NSMakeRect(20, 10, 100, 32)];
...
- (void)strengthChanged:(NSSlider *)sender {
    ...
    _strengthLabel.stringValue = [NSString stringWithFormat:@"%d%%", (int)(_previewStrength * 100)];
}

- (void)applySelectedStyle {
    if (_selectedStyle) {
        if ([self.delegate respondsToSelector:@selector(styleLibrary:didApplyStyle:)]) {
...
- (NSCollectionViewItem *)collectionView:(NSCollectionView *)collectionView itemForRepresentedObjectAtIndexPath:(NSIndexPath *)indexPath {
    NSCollectionViewItem *item = [collectionView makeItemWithIdentifier:@"StyleItem" forIndexPath:indexPath];
    ...
    if (indexPath.item < (NSInteger)_displayedStyles.count) {
        ...
        // Thumbnail
        NSImageView *imageView = [[NSImageView alloc] initWithFrame:NSMakeRect(10, 50, 130, 100)];
        imageView.image = style.thumbnail ?: [NSImage imageNamed:@"style_placeholder"];
        imageView.imageScaling = NSImageScaleProportionallyUpOrDown;
        [view addSubview:imageView];

        // Name label
        NSTextField *nameLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(10, 25, 130, 20)];
        ...
        // Category label
        NSTextField *catLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(10, 5, 130, 16)];
        ...
        // Favorite indicator
        if (style.isFavorite) {
            NSTextField *favIndicator = [[NSTextField alloc] initWithFrame:NSMakeRect(125, 155, 20, 20)];
...
- (void)collectionView:(NSCollectionView *)collectionView didSelectItemsAtIndexPaths:(NSSet<NSIndexPath *> *)indexPaths {
    ...
    if (indexPath && indexPath.item < (NSInteger)_displayedStyles.count) {
        ...
        if ([self.delegate respondsToSelector:@selector(styleLibrary:didSelectStyle:)]) {
