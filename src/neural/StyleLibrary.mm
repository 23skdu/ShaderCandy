...
- (instancetype)init {
  ...
  if (self) {
    ...
    _libraryQueue = dispatch_queue_create("com.shadercandy.stylelibrary",
                                          DISPATCH_QUEUE_SERIAL);

    NSString *appSupport = [@"~/Library/Application Support/ShaderCandy/Styles"
...
- (void)createLibraryDirectory {
  ...
  if (![fm fileExistsAtPath:_libraryPath]) {
    [fm createDirectoryAtPath:_libraryPath
...
- (void)loadFavorites {
  NSString *favoritesPath =
      [_libraryPath stringByAppendingPathComponent:@"favorites.plist"];
...
- (NSArray<StylePreset *> *)scanBundledStyles {
  ...
  NSArray *modelURLs = [bundle URLsForResourcesWithExtension:@"mlmodelc"
                                                subdirectory:@"Styles"];

  for (NSURL *url in modelURLs) {
    NSString *name = url.lastPathComponent.stringByDeletingPathExtension;
    StylePreset *preset = [[StylePreset alloc] initWithIdentifier:name
                                                             name:name
                                                         category:@"Art"];
  ...
  NSDictionary *styleInfo = @{
    ...
    @"monet" :
        @{@"name" : @"Monet", @"category" : @"Art", @"strength" : @(0.8)},
    @"picasso" :
        @{@"name" : @"Picasso", @"category" : @"Art", @"strength" : @(0.85)},
    @"hokusai" : @{
      @"name" : @"Hokusai Wave",
      @"category" : @"Art",
      @"strength" : @(0.8)
    ...
    @"watercolor" :
        @{@"name" : @"Watercolor", @"category" : @"Art", @"strength" : @(0.8)},
    @"sketch" : @{
      @"name" : @"Pencil Sketch",
      @"category" : @"Art",
      @"strength" : @(0.85)
...
- (nullable NSArray<StylePreset *> *)allStyles {
  return [_styles copy];
}

- (nullable NSArray<NSString *> *)categories {
...
- (nullable StylePreset *)styleNamed:(nullable NSString *)name {
  for (StylePreset *style in _styles) {
    if ([style.name isEqualToString:name] ||
        [style.identifier isEqualToString:name]) {
      return style;
    }
  }
  return nil;
...
- (BOOL)downloadStyle:(nullable NSString *)styleName
  ...
  if (completion) {
    completion(NO, [NSError errorWithDomain:@"StyleLibrary"
                                       code:1001
                                   userInfo:@{
...
- (BOOL)importStyleFromURL:(NSURL *)url error:(NSError **)error {
  if (![url.pathExtension isEqualToString:@"mlmodelc"] &&
    ...
    if (error) {
      *error =
          [NSError errorWithDomain:@"StyleLibrary"
                              code:1002
                          userInfo:@{
                            NSLocalizedDescriptionKey : @"Invalid model format"
                          }];
    }
    return NO;
  }
...
- (BOOL)deleteStyle:(StylePreset *)style error:(NSError **)error {
  if (style.isBuiltIn) {
    if (error) {
      *error = [NSError
          errorWithDomain:@"StyleLibrary"
                     code:1003
                 userInfo:@{
                   NSLocalizedDescriptionKey : @"Cannot delete built-in styles"
                 }];
    }
    return NO;
  }

  NSString *path = [_libraryPath
      stringByAppendingPathComponent:[style.identifier
                                         stringByAppendingString:@".mlmodelc"]];
  NSFileManager *fm = [NSFileManager defaultManager];
