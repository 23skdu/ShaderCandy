...
- (void)setupWindow {
  ...
  if (!_shaderNameLabel) {
    _shaderNameLabel = [[NSTextField alloc]
        initWithFrame:NSMakeRect(20, contentView.bounds.size.height - 40, 300,
                                 24)];
    _shaderNameLabel.stringValue = @"ShaderCandy";
    _shaderNameLabel.font = [NSFont systemFontOfSize:16
...
- (NSToolbarItem *)toolbar:(NSToolbar *)toolbar
  ...
  if ([itemIdentifier isEqualToString:@"shaderSelector"]) {
    ...
    NSPopUpButton *popup =
        [[NSPopUpButton alloc] initWithFrame:NSMakeRect(0, 0, 150, 24)];
    ...
    item.image =
        [NSImage imageWithSystemSymbolName:@"arrow.up.left.and.arrow.down.right"
    ...
    item.image = [NSImage imageWithSystemSymbolName:@"square.grid.2x2"
                           accessibilityDescription:@"Shaders"];
    ...
    item.image = [NSImage imageWithSystemSymbolName:@"chart.bar"
                           accessibilityDescription:@"Metrics"];
...
- (void)showShaderList:(BOOL)show {
  ...
  if (show) {
    if (!_shaderListContainer) {
      ...
      _shaderListContainer.autoresizingMask =
          NSViewHeightSizable | NSViewMaxXMargin;
      _shaderListContainer.layer.backgroundColor =
          [NSColor colorWithWhite:0.1 alpha:0.9].CGColor;
    ...
    [NSAnimationContext
        runAnimationGroup:^(NSAnimationContext *context) {
          ...
          _shaderListContainer.animator.frame =
              NSMakeRect(0, 0, 200, contentView.bounds.size.height);
        }
    ...
    if (_shaderListContainer) {
      [NSAnimationContext
          runAnimationGroup:^(NSAnimationContext *context) {
            context.duration = 0.3;
            _shaderListContainer.animator.frame =
                NSMakeRect(-200, 0, 200, contentView.bounds.size.height);
          }
          completionHandler:^{
...
- (void)showMetrics:(BOOL)show {
  ...
  if (show) {
    NSLog(@"Metrics: FPS=%.1f, GPU=%.2fms, Dropped=%ld", _currentFPS, _gpuTime,
          (long)_droppedFrames);
  }
}
...
- (void)setAvailableShaders:(NSArray<NSString *> *)shaders {
  ...
  // Update popup button
  if (_shaderSelectorItem &&
      [_shaderSelectorItem.view isKindOfClass:[NSPopUpButton class]]) {
...
- (void)selectShader:(NSString *)shaderName {
  if (!shaderName || [_currentShader isEqualToString:shaderName])
    return;
  ...
  // Notify delegate
  if ([self.delegate respondsToSelector:@selector(windowController:
                                                   didSelectShader:)]) {
...
- (void)updateMetrics {
  ...
  // Update FPS label
  if (_fpsLabel) {
    _fpsLabel.stringValue =
        [NSString stringWithFormat:@"%.1f FPS", _currentFPS];
  }
