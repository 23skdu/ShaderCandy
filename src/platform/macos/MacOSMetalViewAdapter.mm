...
- (void)commonInit {
    ...
    NSLog(@"MacOSMetalViewAdapter: Initializing with frame %@", NSStringFromRect(self.bounds));
}
...
- (void)setupMetal {
    ...
    // Create renderer
    NSError *error = nil;
    _renderer = [MetalRenderer rendererWithDevice:self.mtkView.device error:&error];
    if (error) {
    ...
    NSLog(@"MacOSMetalViewAdapter: Found %lu shaders", (unsigned long)_availableShaders.count);
...
- (void)loadShaders {
    if (!_renderer || !_currentShaderName) return;

    NSError *error = nil;
    [_renderer setActiveShader:_currentShaderName error:&error];
    if (error) {
        NSLog(@"MacOSMetalViewAdapter: Failed to load shader '%@': %@", _currentShaderName, error);
    }
}
...
- (void)drawInMTKView:(MTKView *)view {
    if (!_renderer || !view.currentDrawable || !view.currentRenderPassDescriptor) return;
...
- (NSWindow *)configureSheet {
    ...
    NSView *contentView = [[NSView alloc] initWithFrame:NSMakeRect(0, 0, 320, 400)];
    [window setContentView:contentView];

    int y = 360;

    // Shader selector
    NSTextField *label = [[NSTextField alloc] initWithFrame:NSMakeRect(20, y, 80, 20)];
    ...
    NSPopUpButton *popup = [[NSPopUpButton alloc] initWithFrame:NSMakeRect(100, y - 2, 200, 25)
                                                     pullsDown:NO];
    ...
    NSSlider *speedSlider = [[NSSlider alloc] initWithFrame:NSMakeRect(100, y, 200, 20)];
    ...
    // Intensity slider
    NSTextField *intLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(20, y, 80, 20)];
    ...
    NSSlider *intSlider = [[NSSlider alloc] initWithFrame:NSMakeRect(100, y, 200, 20)];
    ...
    // Gravity slider
    NSTextField *gravLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(20, y, 80, 20)];
    ...
    NSSlider *gravSlider = [[NSSlider alloc] initWithFrame:NSMakeRect(100, y, 200, 20)];
    ...
    // Bloom toggle
    NSButton *bloomCheck = [[NSButton alloc] initWithFrame:NSMakeRect(100, y, 200, 20)];
    [bloomCheck setButtonType:NSButtonTypeSwitch];
    [bloomCheck setTitle:@"Enable Bloom Glow"];
    [bloomCheck setState:_enableBloom ? NSControlStateValueOn : NSControlStateValueOff];
    ...
    // OK button
    NSButton *okButton = [[NSButton alloc] initWithFrame:NSMakeRect(220, 20, 80, 24)];
