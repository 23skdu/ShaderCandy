...
- (void)createMainWindow {
    ...
    _windowController = [[StandaloneAppWindowController alloc] initWithWindowNibName:@"StandaloneAppWindow"];
    
    // Configure the window
    NSWindow *window = _windowController.window;
    if (!window) {
        // Create window programmatically if nib not found
...
- (void)setupMenuBar {
    ...
    NSMenuItem *appMenuItem = [[NSMenuItem alloc] initWithTitle:@"ShaderCandy" action:NULL keyEquivalent:@""];
    ...
    [_appMenu addItem:[[NSMenuItem alloc] initWithTitle:@"Hide Others"
                                                  action:@selector(hideOtherApplications:)
                                           keyEquivalent:@"h"]];
    
    [_appMenu addItem:[[NSMenuItem alloc] initWithTitle:@"Show All"
    ...
    [_viewMenu addItem:[[NSMenuItem alloc] initWithTitle:@"Show Shader List"
    ...
    [_viewMenu addItem:[[NSMenuItem alloc] initWithTitle:@"Enter Full Screen"
                                                   action:@selector(toggleFullScreen)
                                            keyEquivalent:@"f"]];
    
    // Settings menu
    _settingsMenu = [[NSMenu alloc] initWithTitle:@"Settings"];
    NSMenuItem *settingsMenuItem = [[NSMenuItem alloc] initWithTitle:@"Settings" action:NULL keyEquivalent:@""];
    ...
    [_settingsMenu addItem:[[NSMenuItem alloc] initWithTitle:@"Enable Audio Reactivity"
                                                        action:NULL
                                                 keyEquivalent:@""]];
...
- (void)loadInitialShader {
    if (!_renderer || !_currentShader) return;
    
    NSError *error = nil;
    BOOL success = [_renderer setActiveShader:_currentShader error:&error];
    if (!success) {
        NSLog(@"Failed to load initial shader '%@': %@", _currentShader, error.localizedDescription);
...
- (BOOL)switchToShader:(NSString *)shaderName {
    ...
    if (success) {
        ...
        NSLog(@"Failed to switch to shader '%@': %@", shaderName, error.localizedDescription);
    }
...
- (void)showAboutPanel {
    ...
    NSString *aboutText = [NSString stringWithFormat:
        @"%@\nVersion %@\n\nShaderCandy is a cross-platform screensaver engine that renders real-time procedural graphics using native GPU APIs.\n\n© 2024 ShaderCandy",
        appName, version];
...
- (void)showPreferences {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ShowPreferences" object:nil];
}
...
- (void)openShaderFile {
    ...
    openPanel.allowedContentTypes = @[@"public.source-code", @"public.text"];
    openPanel.directoryURL = [NSURL fileURLWithPath:[NSString stringWithUTF8String:ShaderCandy::Config::ConfigurationManager::getShaderDirectory().c_str()]];
    
    if ([openPanel runModal] == NSModalResponseOK) {
        NSURL *selectedURL = openPanel.URL;
        NSString *shaderName = selectedURL.lastPathComponent.stringByDeletingPathExtension;
        [self switchToShader:shaderName];
    }
}

- (void)exportPreset {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ExportPreset" object:nil];
}
...
- (void)importPreset {
    ...
    openPanel.canChooseDirectories = NO;
    openPanel.allowedContentTypes = @[@"public.json"];
...
- (void)toggleShaderList {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ToggleShaderList" object:nil];
}

- (void)toggleMetrics {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ToggleMetrics" object:nil];
}
...
- (void)showHelp {
    // Open documentation
    NSString *docsPath = [[NSBundle mainBundle] pathForResource:@"README" ofType:@"md"];
    if (docsPath) {
...
- (void)showShortcuts {
    ...
    alert.informativeText = @"⌘L - Show Shader List\n⌘M - Show Metrics\n⌘F - Full Screen\n⌘, - Preferences\n⌘Q - Quit\n\nShader Controls:\nSpace - Next Shader\n←/→ - Previous/Next Shader\n↑/↓ - Speed +/-";
    [alert addButtonWithTitle:@"OK"];
    [alert runModal];
}
